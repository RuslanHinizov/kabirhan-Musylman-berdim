...
import type { AnalyticsCameraState } from '../../store/cameraStore';
...
// Memoized camera tile — re-renders only when its own props change
...
const CameraTile = memo(({ camera, horsesNear, streamState, fps, isExpanded, livePreviewMode, gridIntervalMs, onToggleExpand }: CameraTileProps) => {
    const { t } = useTranslation();
    const isOnline = streamState === 'running' || streamState === 'connecting';
    const useLivePreview = livePreviewMode && streamState === 'running' && !isExpanded;
        ...
        >
            <div className="aspect-video bg-black relative">
                {useLivePreview ? (
                    ...
                    <SnapshotPlayer
                        url={`${BACKEND_HTTP_URL}/snapshot/${camera.id}?quality=50&width=640`}
                        cameraName={camera.name}
                        className="w-full h-full"
                ...
                {isOnline && fps > 0 && (
                    <div className="absolute top-2 left-2 z-10">
                        <div className="px-1.5 py-0.5 rounded bg-black/70 text-[10px] font-mono font-bold text-emerald-400">
                            {fps.toFixed(0)} FPS
                        </div>
                    </div>
                )}
...
export const CameraGrid = () => {
    ...
    const [streamFps, setStreamFps] = useState<Record<string, number>>({});

    const fetchStreamStates = useCallback(async () => {
        try {
            const res = await fetch(`${BACKEND_HTTP_URL}/api/streams/status`);
            const status = await res.json() as Record<string, { state?: string; fps?: number }>;
            const nextState: Record<string, string> = {};
            const nextFps: Record<string, number> = {};
            Object.entries(status).forEach(([camId, info]) => {
                nextState[camId] = info?.state || 'idle';
                nextFps[camId] = info?.fps || 0;
            });
            setStreamStates(nextState);
    ...
    // Pre-compute horses per camera once — avoids N×M filter in each tile render
    const horsesPerCamera = useMemo(() => {
        const map: Record<string, Horse[]> = {};
        for (const cam of analyticsCameras) {
            map[cam.id] = rankings.filter(horse => {
                const horsePos = horse.distanceCovered % TRACK_LENGTH;
                return horsePos >= cam.trackStart && horsePos < cam.trackEnd;
            });
        }
        return map;
    }, [analyticsCameras, rankings]);
    ...
    return (
        <div className="p-6 h-full flex flex-col overflow-hidden">
            ...
            {expandedCamera && (() => {
                ...
                const horsesNear = horsesPerCamera[camera.id] || [];
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 flex-1 overflow-y-auto p-1">
                {visibleGridCameras.map((camera) => (
                    <CameraTile
                        key={camera.id}
                        camera={camera}
                        horsesNear={horsesPerCamera[camera.id] || []}
                        streamState={streamStates[camera.id] || 'idle'}
                        fps={streamFps[camera.id] || 0}
